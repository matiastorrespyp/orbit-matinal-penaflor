...
### PENDIENTE
- [x] **CAUSA RAIZ ARREGLADA — el autodescubrimiento ya excluye el mes en curso.** El loop de descubrimiento saltea `periodo >= _now_ar()[:7]` (hora argentina, no `date.today()`, que en Render/UTC se corre en los bordes de mes). Se agrego ademas el periodo actual a la huella de `_cierres_hist_key()`: sin eso, el 1ro de mes el cierre recien terminado quedaba oculto detras de la cache. Validado con el archivo de agosto restaurado: excluido como `_082026`, visible al renombrarlo a `_042026`.
...
### HECHO
- [x] **Explorador unico**: 3 selectores (Categoria -> Linea -> Segmento) y **un solo panel** que se reusa. Reemplaza el mosaico de tarjetas de reglas. Gerencia y vendedor comparten el mismo bloque (`accxHTML` / `accxPanel` en portal.html).
- [x] **El Excel manda**: `generar_acciones_explorador()` lee las 8 hojas del libro del mes y escribe `04_DATASETS_ORBIT/mod_acciones_explorador.json`. Cero reglas cableadas en el portal; sirve para cualquier carpeta `YYYY-MM`.
- [x] **Dataset y no lectura del xlsx en vivo**: el `.bat` solo publica los `.csv` de esa carpeta, el `.xlsx` no llega a Render (ERR-014). Se agrego `git add "04_DATASETS_ORBIT/mod_acciones_explorador.json"` al cierre, CRLF preservado.
...
- [x] **Regla definitiva de V20/Deposito**: **EMPRESA** incluye la venta directa; **VENDEDORES** son solo los de ruta. V20 no es vendedor activo, no tiene cartera, no va a rankings ni selectores y no genera cumplimiento individual, pero **sus ventas validas suman al total de 11T de la distribuidora**. Escrito en `CLAUDE.md`.
- [x] **Correccion semantica: `DEPOSITO` != `SIN_CARTERA`.** Cuatro categorias mutuamente excluyentes (`universo`): `VENDEDORES`, `DEPOSITO` (V1/V20), `SIN_CARTERA` (sin `codven`) y `BAJA` (V2/V5, fuera de todo). **DEPOSITO es una decision comercial; SIN_CARTERA es un hueco del ERP.** Confundirlos hacia pasar a `#786 ANSELMI` por venta directa legitima y el hueco no se arreglaba nunca.
- [x] **Trazabilidad de SIN_CARTERA**: `mod_11t_sin_cartera.csv`, excepcion `CLIENTE_SIN_CARTERA` con los codigos de cliente, aviso `[REVISAR]` en la regeneracion y `sin_cartera_clientes` en `/api/gerencia/once_titulares`.
- [x] **Tests especificos de doble conteo**: `cubiertos_empresa = cubiertos_vendedores + cubiertos_deposito + cubiertos_sin_cartera` en sintetico, en datos reales y en los endpoints; los universos no comparten ningun cliente. **96 tests OK** en los dos modulos del motor.

### PENDIENTE
- [ ] **Recalibrar los objetivos de `objetivo 11T.xlsx` con Peñaflor.** **No se tocaron** en estos commits y no hay que tocarlos hasta que el negocio confirme. Se fijaron contra la regla vieja (CCC sin minimo de botellas); con cobertura real Gordon's queda en **61,5%** del objetivo 52. Numerador y denominador ya miden lo mismo. **Es la decision que mas mueve la lectura de la pantalla.**
- [ ] **`#786 ANSELMI Y CIA` — decidir con el negocio y corregir en el ERP.** Es el caso testigo de `SIN_CARTERA`: Autoservicio grande, **sin `codven`** en el padron, 1.560 botellas de Smirnoff Flavours en julio y 9 titulares cubiertos. Hoy suma al total de empresa y queda fuera de todo ranking, que es lo correcto mientras no se sepa. **No asignarlo a un vendedor ni a Deposito sin evidencia del ERP.** Si resulta ser venta directa, se le pone `codven` 1/20 y pasa a `DEPOSITO`; si es de una ruta, se le asigna cartera y pasa a `VENDEDORES` solo. Lo mismo con **`#20010 SENN ENZO`** (7 botellas, mucho menos material).
- [ ] **Los otros clientes sin `codven` del padron** (9 en total) todavia no aparecieron con compras 11T. Revisar `mod_11t_sin_cartera.csv` despues de cada cierre: si aparece uno nuevo, es una alta que quedo sin cartera en el ERP.
- [ ] **Los 27 clientes `codven=1` (`deposito`) no tuvieron compras 11T en julio**, asi que el universo `DEPOSITO` aporta 0 hoy. Vale mirar un mes con movimiento de mostrador para ver su aporte real antes de dar la regla por cerrada con datos.
- [ ] **`04_DATASETS_ORBIT/mod_11t_acum.csv` tiene columnas nuevas (`universo`, `cuenta_vendedor`) y hay un dataset nuevo (`mod_11t_sin_cartera.csv`)**: hay que **regenerarlos con el cierre** para que Render los tome. Los consumidores tienen fallback si falta la columna, pero mientras tanto la apertura por universo no se ve en el portal publicado. El allowlist de `CIERRE_DIA_ORBIT.bat` ya cubre los tres datasets nuevos del 11T, asi que el proximo cierre los publica solo. Verificar despues con `git status --short 04_DATASETS_ORBIT/`: lo que siga modificado no llego a Render.
- [ ] **`cargar_clientes()` de `generar_datasets_acum` no descarta los `codven` vacios**, asi que un cliente sin cartera puede agruparse bajo un vendedor `NaN` en cobertura y otros datasets. En el 11T ya esta resuelto; **fuera del 11T sigue pendiente** y no se toco para no mover denominadores de otras tarjetas en este cambio.
...
- [x] **El padrón viaja con el cierre diario**: `git add "01_INPUTS/Plan cobertura"` agregado al allowlist de `CIERRE_DIA_ORBIT.bat` (el negocio lo va a ir editando a medida que se den de alta clientes). Los `~$` de Excel quedaron gitignoreados para que el staging de la carpeta no se lleve el archivo de bloqueo si el xlsx está abierto.
- [x] **Buscador (lupa)** arriba de la pantalla: filtra las cuatro listas a la vez por cliente / localidad / vendedor, admite varias palabras, y cada tarjeta muestra `N de M` mientras hay filtro. Render parcial (`pcFiltrar` re-pinta sólo los tbody) para no perder el foco del input.
...
### HECHO
- [x] **Los 10 clientes duplicados quedan en V8** (criterio: la vendedora que le supo vender; verificado contra el historial 2024-04 → 2026-07). Borradas las 10 filas de `codven = 3` de `clientes.xlsx`: un cliente, una fila. Cartera Tradicional 1.693 → 1.684, V3 293 → 284.
- [x] **Guard anti-duplicados** en los dos loaders del maestro: si el ERP vuelve a exportar un cliente en dos rutas, se deja una fila y se imprime `[AVISO]` con los códigos. No adivina el dueño — corta la duplicación y pide corregir el ERP.
...
- [ ] **Confirmar el traspaso de ruta V3 → V8.** Los 10 duplicados quedaron asignados a **V8** por historial de ventas (V8 es la última que le vendió a los 10; 3 nunca le compraron a V3). En `#272` y `#1065` V3 tiene más historia (16-17 meses) pero cortada en junio 2026: si no hubo traspaso de ruta, son los dos a revisar.
...
### RESUELTO
- [x] **Objetivo Tradicional 845 vs 809:** el usuario actualizó la hoja `tradicional` de objccc.xlsx (2026-07-21) repartiendo los objetivos por vendedor → ahora suman **845** exacto (V3 116, V4 118, V6 111, V7 116, V8 125, V9 117, V10 142). `objetivo_asignado == objetivo_total == 1046`; la leyenda "sin asignar" del pie se oculta sola. Validado en navegador + endpoint. Excel pusheado a Render.
### HECHO (cont.)
