...
### PENDIENTE
- [ ] **17 PDV atendidos sin código de cliente cargado** en el padrón: hasta que se complete la columna `CÓD. CLIENTE` no se les puede medir activación ni recompra. La pantalla ya **propone candidatos por dirección** (8 de 17 tienen, 1 con confianza alta: `Maxikiosco Porteña` → `#4405`), con la última compra de cada uno para confirmarlos. Falta que el negocio los valide y escriba el código en el Excel. **No se asignan solos a propósito**: sobre los 17 casos reales el cruce automático sólo acierta 1, el resto son vecinos de la misma calle.
