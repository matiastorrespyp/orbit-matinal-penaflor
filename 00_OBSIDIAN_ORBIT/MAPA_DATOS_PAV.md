...
## ⚠️ orbit.db NO persiste en Render (2026-08-07 → hasta la migración a Postgres)

El servicio de Render **no tiene disco persistente** y **no tiene `ORBIT_DB_PATH`**: `DB_PATH` cae a `BASE/orbit.db`, filesystem **efímero**, y `orbit.db` está gitignoreado. **Cada deploy arranca con la base vacía.**

| Tabla | Qué guarda | ¿Sobrevive al deploy? |
|---|---|---|
| `planificacion` | Planes de vendedores | **Sí** — la fuente de verdad es Google Sheets |
| `cierre_plan_accion` | Plan de acción de la reunión | **No** |
| `plan_semanal` | Planificación semanal por KPI | **No** |
| `plan_cob_nota` | Mensajes por PDV de Plan Cobertura | **No** |

Decisión del usuario: **no se paga el disco**; este mes se migra a **VPS + Postgres** y luego a la **API de GescomWeb**. Detalle y diagnóstico en **ERR-015** ([[05_ERRORES_Y_SOLUCIONES/README]]) y [[BITACORA_2026-08-07]].
...
| /api/gerencia/cierres_historicos | OK | `07_CIERRES_MENSUALES/` + `01_INPUTS/cierres mes/` | **Cacheado por mtime** (índice + árbol 07 + trío versionado + maestro 04D). Render: ~23 s en frío, **0,2–0,3 s** después. El armado vive en `_cierres_historicos()`; la ruta sólo hace `jsonify`. Ver [[BITACORA_2026-08-06b]] |
| /api/gerencia/cierre_plan_accion | ⚠️ **NO PERSISTE** — SQLite | mismo caché de cierres + orbit.db (`cierre_plan_accion`) | `GET ?periodo=YYYY-MM` → `pendientes` (no logrados del mes) + `seguimiento` (plan del mes anterior con resultado **medido**). `POST` guarda `{periodo, autor, acciones:{id: texto}}`; texto vacío borra. Ver [[BITACORA_2026-08-06b]] |
