...
| /api/gerencia/cierre_plan_accion | OK — SQLite | mismo caché de cierres + orbit.db (`cierre_plan_accion`) | `GET ?periodo=YYYY-MM` → `pendientes` (no logrados del mes) + `seguimiento` (plan del mes anterior con resultado **medido**). `POST` guarda `{periodo, autor, acciones:{id: texto}}`; texto vacío borra. Ver [[BITACORA_2026-08-06b]] |
