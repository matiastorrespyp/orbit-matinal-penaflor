...
def _cierre_ranking_payload(rank):
    """Transforma la lista de ranking (shape ranking_vendedores_mes.json) en
    {ranking_top3, ranking, ganadores} como las espera el portal."""
...
def _cierre_extras_versionado(files):
    ...
    reutilizando el motor oficial (tools/generar_cierre_mensual.py). Catálogos compartidos:
    04D_MAESTRO_PRODUCTOS_PENAFLOR.xlsx, INNOVACIONES/Innovaciones.xlsx, vendedores_activos.csv."""
    out = {}
    # Sell-Out (mismo cálculo que el endpoint live, sobre el ventas_mes versionado)
    try:
        so_df = _leer_ventas_mes_csv(files["ventas_mes"])
        out["sellout"] = {
    ...
    try:
        gcm = _gcm()
        df = gcm._leer_ventas(files["ventas_mes"])
        ...
        m04d = gcm._leer_04d(INPUTS / "04D_MAESTRO_PRODUCTOS_PENAFLOR.xlsx")

        # Innovaciones por producto (CCC Mes = clientes únicos con compra en el mes)
        ...
        por_producto = [{
            ...
            "compraron": comp, "pct": round(comp / ccc_mes * 100, 1) if ccc_mes else 0.0,
        } for (cod, nom), comp in prod.items()]
        por_producto.sort(key=lambda x: -x["pct"])
        out["innovaciones"] = {
            "resumen": {
                "productos":       len(por_producto),
                "compraron_total": sum(p["compraron"] for p in por_producto),
                "ccc_mes":         ccc_mes,
        ...
        # Ranking
        dfl    = gcm._litros(df, m04d)
        litros = dfl.groupby("CodVendedor")["litros"].sum().to_dict()
        dinero = df.groupby("CodVendedor")["ImporteNetoItem"].sum().to_dict()
        c11t   = gcm._11t_por_vend(df)
        inov   = gcm._inov_por_vend(df, cod_inov)
...
def gerencia_cierres_historicos():
    ...
    for entrada in indice:
        ...
        if _files:
            ...
            # Sell-Out + Innovaciones + Ranking desde el ventas_mes versionado (FASE 2)
            try:
